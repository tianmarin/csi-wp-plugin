...
class NOVIS_CSI_PROJECT_RISK_CLASS extends NOVIS_CSI_CLASS{
...
public function __construct(){
	...
	$this->class_name	= 'project_risk';
	//Nombre singular para títulos, mensajes a usuario, etc.
	$this->name_single	= 'Riesgo de Proyecto';
	//Nombre plural para títulos, mensajes a usuario, etc.
	$this->name_plural	= 'Riesgos de Proyecto';
	...
	$this->crt_tbl_sql_wt	="
		(
			...
			text tinytext not null COMMENT 'Risk text',
			flag_resolved tinyint(1) null COMMENT 'Indicates if risk has been resolved',
	...
	add_action( 'wp_ajax_csi_fetch_project_risk_list_info',		array( $this , 'csi_fetch_project_risk_list_info'));
	add_action( 'wp_ajax_csi_create_project_risk',				array( $this , 'csi_create_project_risk'		));
	add_action( 'wp_ajax_csi_create_project_risk_form',			array( $this , 'csi_create_project_risk_form'	));
	add_action( 'wp_ajax_csi_edit_project_risk',				array( $this , 'csi_edit_project_risk'			));
	add_action( 'wp_ajax_csi_edit_project_risk_form',			array( $this , 'csi_edit_project_risk_form'		));
	add_action( 'wp_ajax_csi_delete_project_risk',				array( $this , 'csi_delete_project_risk'		));
...
public function csi_define_capabilities(){
	...
	if ( FALSE === $key ) {
		$class_cap = array(
			'name'	=> $cap_group,
			'caps'	=> array(
				'csi_pm_manage_project_risks',
			),
		);
		array_push ( $csi_capabilities, $class_cap);
	}else{
		array_push ( $csi_capabilities[$key]['caps'] ,'csi_pm_manage_project_risks' );
	}
}
public function csi_fetch_project_risk_list_info(){
	//Globa Variables
	global $wpdb;
	...
	$total_risks= $wpdb->get_var ( $count_sql );
	$page_count	= ceil ( $total_risks / $page_size );
	...
	';
	$project_risks = $this->get_sql ( $sql );
	//--------------------------------------------------------------------------
	if ( current_user_can_for_blog( 1, 'csi_pm_manage_project_risks') ){
		$o.='
		<tr class="hidden-print">
			<td colspan="999" class="">
				<a href="#" class="in-table-form-button btn btn-warning btn-xs" data-action="csi_create_project_risk_form" data-project-id="' . $project_id . '">
	...
	foreach ( $project_risks as $project_risk ){
		//----------------------------------------------------------------------
		$reg_date = new DateTime ( $project_risk['reg_date'] );
		//----------------------------------------------------------------------
		$o.='
			<tr>
				<td class="col-xs-1">';
		if ( current_user_can_for_blog( 1, 'csi_pm_manage_project_risks') ){
			$o.='
					<a href="#" class="in-table-form-button hidden-print" data-action="csi_edit_project_risk_form" data-risk-id="' . $project_risk['id'] . '">
		...
		$o.='	</td>
				...
				<td class="col-xs-1"><i class="fa fa-fw fa-' . ( $project_risk['flag_resolved'] ? 'check-square-o' : 'square-o' ) . '"></i></td>
				<td class="small col-xs-8">' .  nl2br ( $project_risk['text'] ) . '</td>
			</tr>
		';
	}
	//Pagination
...
public function csi_create_project_risk(){
	//Globa Variables
	global $wpdb;
	//Local Variables
	$insertArray			= array();
	...
	echo json_encode($response);
	wp_die();
}// csi_create_project_risk

public function csi_create_project_risk_form(){
	//Globa Variables
	global $wpdb;
	//Local Variables
	$o						= '';
	...
	if ( current_user_can_for_blog( 1, 'csi_pm_manage_project_risks') ){
		//----------------------------------------------------------------------
		$o='<div class="row">
			<form class="in-table-form" data-function="csi_create_project_risk">
				...
				<div class="col-xs-12">
					<div class="text-right">
						...
						<button type="submit" class="btn btn-warning"><i class="fa fa-plus"></i> Agregar riesgo</button>
					</div>
				</div>
			</form>
			</div>
	...
	echo json_encode($response);
	wp_die();
}// csi_create_project_risk_form

public function csi_edit_project_risk(){
	//Globa Variables
	global $wpdb;
	//Local Variables
	$editArray			= array();
	...
	$reg_date			= new DateTime ( $post['reg_date']);
	if ( current_user_can_for_blog( 1, 'csi_pm_manage_project_risks') ){
		$whereArray['id']						= intval ( $post['project_risk_id'] );

		$editArray['reg_date']					= $reg_date->format('Y-m-d');
		$editArray['text']						= strip_tags(stripslashes( $post['text'] ));
	...
	echo json_encode($response);
	wp_die();
}// csi_edit_project_risk

public function csi_edit_project_risk_form(){
	//Globa Variables
	global $wpdb;
	//Local Variables
	$date_time_input_opts	='';
	...
	if ( current_user_can_for_blog( 1, 'csi_pm_manage_project_risks') ){
		//----------------------------------------------------------------------
		$sql ='SELECT * FROM ' . $this->tbl_name . ' WHERE id="' . $post['riskId'] . '"';
		$risk = $wpdb->get_row ( $sql );
		//----------------------------------------------------------------------
		$o='<div class="row">
			<form class="in-table-form" data-function="csi_edit_project_risk">
				<input type="hidden" name="project_risk_id" value="' . $risk->id . '"/>
				<div class="col-sm-1">&nbsp;</div>
				<div class="col-sm-2">
					<div class="row">
						<label for="reg_date" class="col-xs-12 visible-xs">Fecha</label>
						<div class="col-xs-12">
							<input type="text" name="reg_date" id="reg_date" class="form-control input-sm csi-date-range-input" required="true" value="' . $risk->reg_date . '" data-single-date-picker="true" data-drops="up"/>
							<p class="help-block"></p>
						</div>
					</div>
				</div>
				<div class="col-sm-1">
					<div class="row">
						<label class="col-xs-12 visible-xs">Resoluci&oacute;n</label>
						<div class="col-xs-12">
							<input type="checkbox" class="form-control csi-cool-checkbox" id="flag_resolved" name="flag_resolved" value="1" ' . ( $risk->flag_resolved ? 'checked' : '' ) . '/>
							<label for="flag_resolved">&nbsp;</label>
							<p class="help-block"></p>
						</div>
					</div>
				...
				<div class="col-sm-8">
					<div class="row">
						<label for="text" class="col-xs-12 visible-xs">Texto</label>
						<div class="col-xs-12">
							<textarea name="text" id="text" class="form-control input-sm" required="true" maxlength="255">' . $risk->text . '</textarea>
							<p class="help-block small">Tama&ntilde;o m&aacute;ximo: 255 caracteres</p>
						</div>
					</div>
				</div>
				<div class="col-xs-12">
					<div class="text-right">
						<div class="pull-left">
							<button type="button" class="btn btn-danger in-table-form-delete" data-action="csi_delete_project_risk" data-content="¿Est&aacute;s seguro que quieres eliminar este riesgo del proyecto? <pre>' . $risk->text . '</pre>" data-type="red" data-column-class="xlarge">
						...
						<button type="submit" class="btn btn-warning">
							<i class="fa fa-pencil"></i> Editar riesgo
						</button>
					</div>
				</div>
			</form>
	...
	echo json_encode($response);
	wp_die();
}// csi_edit_project_risk_form

public function csi_delete_project_risk(){
	//Globa Variables
	global $wpdb;
	//Local Variables
	$editArray			= array();
...
global $NOVIS_CSI_PROJECT_RISK;
$NOVIS_CSI_PROJECT_RISK =new NOVIS_CSI_PROJECT_RISK_CLASS();
?>
